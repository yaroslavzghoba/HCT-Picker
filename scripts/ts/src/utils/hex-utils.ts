class HexUtils {
    ...
    static validateNumHex(hex: number) {
    ...
    // Throw an error if the hex color code is not valid.
    static validateStrHex(hex: string) {
    ...
    static isNumHexValid(hex: number): boolean {
        return !(typeof hex !== "number" || hex < 0x000000 || hex > 0xffffff || isNaN(hex));
    };

    // Validate a hexidecimal color code and return a boolean value.
    static isStrHexValid(hex: string): boolean {
    ...
    static stringToHex(hex: string): number {
